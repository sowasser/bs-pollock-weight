...
cpue_final2 <- cpue2[,c("year", "age","cruisejoin", "haul", "hauljoin", "start_latitude", "start_longitude", "age_bin", "age_cpue_corr", "weight")]
...
out = tapply( Data[,'age_cpue_sum'], INDEX=list(Data[,'year'],Data[,'age_bin']), FUN=function(x){sum(x==0)} )
# FAILS CHECK

# Sanity check
# Check that sample size is equal for all ages in a given year
out2 = tapply( Data[,'age_cpue_sum'], INDEX=list(Data[,'year'],Data[,'age_bin']), FUN=length )
# FAILS CHECK
...
Data_fix = tapply( Data[,'age_cpue_sum'], INDEX=list("hauljoin"=Data[,'hauljoin'],"age_bin"=Data[,'age_bin']), FUN=sum )
Data_fix = ifelse( is.na(Data_fix), 0, Data_fix )

# rebuild wide-form data frame
Data_wide = cbind( Data_fix, Data[match(rownames(Data_fix),Data[,'hauljoin']),c("start_latitude","start_longitude","year")] )

# build long-form data frame
Data_long = expand.grid( dimnames(Data_fix) )
Data_long = cbind( Data_long, "age_cpue_sum"=as.vector(Data_fix), Data[match(Data_long[,'hauljoin'],Data[,'hauljoin']),c("start_latitude","start_longitude","year")] )

# Sanity check
# Check that there's some zeros
out = tapply( Data_long[,'age_cpue_sum'], INDEX=list(Data_long[,'year'],Data_long[,'age_bin']), FUN=function(x){sum(x==0)} )
# PASSES CHECK

# Sanity check
# Check that sample size is equal for all ages in a given year
out2 = tapply( Data_long[,'age_cpue_sum'], INDEX=list(Data_long[,'year'],Data_long[,'age_bin']), FUN=length )
# PASSES CHECK
...
data2$location <- ifelse(data2$stratum==81 | data2$stratum==70 |data2$stratum==71,'northern','eastern')
...
plot(data2$weight_calculated~data2$weight, ylab="Calculated weight (w=aL^b)", xlab="Measured weight")
abline(a=0,b=1)
